...
def in_loop(maze: Maze, loop: set[tuple[int, int]], pos: tuple[int, int]) -> bool:
    """Determine if a position is contained by the loop."""
    return (
        is_odd(left_vertical_loop(maze, loop, pos))
        and is_odd(right_vertical_loop(maze, loop, pos))
        and is_odd(above_horizontal_loop(maze, loop, pos))
        and is_odd(below_horizontal_loop(maze, loop, pos))
    )


def left_vertical_loop(
    maze: Maze, loop: set[tuple[int, int]], pos: tuple[int, int]
) -> int:
    """Count the number of left, vertical tiles that are part of the loop."""
    row = pos[0]
    col = pos[1] - 1
    ...
    while col >= 0:
        ...
        if (row, col) in loop and tile == Tile.VERTICAL_PIPE:
            count += 1
        col -= 1
...
def right_vertical_loop(
    maze: Maze, loop: set[tuple[int, int]], pos: tuple[int, int]
) -> int:
    """Count the number of right, vertical tiles that are part of the loop."""
    row = pos[0]
    col = pos[1] + 1

    count = 0
    while col < len(maze.tiles[0]):
        tile = maze.tile((row, col))
        if (row, col) in loop and tile == Tile.VERTICAL_PIPE:
            count += 1
        col += 1
...
def above_horizontal_loop(
    maze: Maze, loop: set[tuple[int, int]], pos: tuple[int, int]
) -> int:
    """Count the number of above, horiztonal tiles that are part of the loop."""
    row = pos[0] - 1
    col = pos[1]
    ...
    while row >= 0:
        tile = maze.tile((row, col))
        if (row, col) in loop and tile == Tile.HORIZONTAL_PIPE:
            count += 1
        row -= 1
...
def below_horizontal_loop(
    maze: Maze, loop: set[tuple[int, int]], pos: tuple[int, int]
) -> int:
    """Count the number of below, horiztonal tiles that are part of the loop."""
    row = pos[0] + 1
    col = pos[1]

    count = 0
    while row < len(maze.tiles) - 1:
        tile = maze.tile((row, col))
        if (row, col) in loop and tile == Tile.HORIZONTAL_PIPE:
            count += 1
        row += 1
