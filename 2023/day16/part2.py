...
def n_energized(grid: list[str], initial_beam: Beam) -> int:
    """Compute energized cells."""
    energized: set[tuple[int, int]] = set()

    in_progress: list[Beam] = [initial_beam]
...
def max_energized(grid: list[str]) -> int:
    """Determine the maximum energy."""
    candidates = (
        [Beam(pos=(0, j), dir=Direction.DOWN) for j in range(len(grid[0]))]
        + [Beam(pos=(len(grid) - 1, j), dir=Direction.UP) for j in range(len(grid[0]))]
        + [Beam(pos=(i, 0), dir=Direction.RIGHT) for i in range(len(grid))]
        + [
            Beam(pos=(i, len(grid[0]) - 1), dir=Direction.LEFT)
            for i in range(len(grid))
        ]
    )
    return max(n_energized(grid, beam) for beam in candidates)
