...
class Hand:
    ...
    def type(self) -> int:
        ...
        n_jokers = 0 if 1 not in cards else cards[1]
        has_jokers = True if n_jokers > 0 else False

        # Delete jokers if present
        if has_jokers:
            del cards[1]

        n_combos = len(cards)
        max_cardinality = (
            n_jokers if len(cards) == 0 else max(cards.values()) + n_jokers
        )
