...
def compute_prev_value(seq: np.ndarray) -> int:
    """Compute the previous value for a sequence."""
    _diff = diff(seq)
    if np.all(seq == 0):
        return 0

    return seq[0] - compute_prev_value(_diff)
...
def solve(path: Path) -> None:
    ...
    solution = sum(compute_prev_value(seq) for seq in seqs)
    print(solution)
