...
def expand_universe_by(
    galaxies: list[tuple[int, int]], rows: list[int], cols: list[int], factor: int = 1
) -> list[tuple[int, int]]:
    ...
    for galaxy in galaxies:
        ...
        g.append((galaxy[0] + nrow * (factor - 1), galaxy[1] + ncol * (factor - 1)))
    return g
...
def solve(path: Path) -> None:
    ...
    galaxies = expand_universe_by(parse_galaxies(map), rows, cols, 1000000)
